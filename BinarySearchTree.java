...
public class BinarySearchTree {
    ...
    public void addToChildren(int valueToAdd) {
        if (root == null) {
            System.out.println("Tree is empty.");
            return;
        }
        List<Integer> updatedValues = new ArrayList<>();
        updatedValues.add(root.value); // Keep the root value unchanged
        collectValues(root.left, updatedValues, valueToAdd); // Process left subtree
        collectValues(root.right, updatedValues, valueToAdd); // Process right subtree
        root = null; // Clear the tree
        for (int value : updatedValues) {
            insert(value); // Reconstruct tree with updated values
        }
    }

    private void collectValues(Node node, List<Integer> values, int valueToAdd) {
        if (node == null) {
            return;
        }
        // Add updated value for child nodes
        values.add(node.value + valueToAdd);
        collectValues(node.left, values, valueToAdd);
        collectValues(node.right, values, valueToAdd);
    }
